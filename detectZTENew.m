function detected = detectZTENew(approach, rxSignal, simPara, seqPara)
    global fid;
    ...
    for i = 1:length(U)
        ...
        for p = 1:length(peak)
            ...
            for b = 1:length(Bcs)
                ...
                if (K>2 && isempty(idxU1))
                    peak(p).used = true;
                    continue;
                end

                % find delta_2
                possibleU2_0 = [mod(leftmostLoc1 + expect + (K/2)*Nzc, Nzc*K), ...
                    mod(leftmostLoc1 - expect - (K/2)*Nzc, Nzc*K)]
                ...
                tauLate = mod(info.l1(tidx) - leftmostLoc1 + 1, Nzc);
                fprintf(fid, 'tauLate = %d\n', tauLate);
                fprintf(fid, 'hit info(%d), 11 > left1 = %d\n', ...
                        tidx, info.l1(tidx) > leftmostLoc1);

                % compensation for fractional tau and frequency offset
                rx_ref = circshift(rx, -tauLate);
                rx_ref = circshift(rx_ref, info.l1((tidx)));
                fprintf(fid, 'l1 = %d, left1 = %d\n', ...
                        info.l1(tidx), leftmostLoc1);
...
function [indices, walk] = peakGroup(peak, loc0, PDP, Ncs, Nzc, K)
    indices = []; walk = loc0;
%     fprintf('locL: %d, locR: %d\n', ...
%         mod(nextStep(loc0, Ncs, Nzc, '-'), Nzc*K), ...
%         mod(nextStep(loc0, Ncs, Nzc, '+'), Nzc*K));
    ...
    while (~isempty(find([peak.loc] == mod(nextStep(walk, Ncs, Nzc, '+'), Nzc*K))) && n <= (K/2))
%         fprintf('from locR: %d, next %d\n', walk, mod(nextStep(walk, Ncs, Nzc, '+'), Nzc*K));
